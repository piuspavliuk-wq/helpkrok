...
// Конфігурація для 6 тестів з таймерами (зменшені для швидшої роботи)
const testsConfig = [
  { year: 2024, identifier: '2024-pharmacy-2d', title: '2024р 2д', timerSeconds: 15 },
  { year: 2024, identifier: '2024-pharmacy-eng', title: '2024 рік (Eng)', timerSeconds: 30 },
  { year: 2024, identifier: '2024-pharmacy-text', title: '2024 рік (TEXT)', timerSeconds: 45 },
  { year: 2025, identifier: '2025-pharmacy-february', title: '2025 лютий', timerSeconds: 60 },
  { year: 2025, identifier: '2025-pharmacy-new-ct', title: 'Нові тести з цт 2025', timerSeconds: 75 },
  { year: 2025, identifier: '2025-pharmacy', title: '2025 рік', timerSeconds: 90 }
];

class PharmacyScraperWithTimer {
  ...
  async waitWithTimer(seconds) {
    console.log(`\n⏰ ТАЙМЕР ${seconds} СЕКУНД (${Math.floor(seconds/60)} хв ${seconds%60} с) ДЛЯ РУЧНОГО НАЛАШТУВАННЯ (${this.testConfig.title})`);
  ...
  async clickOnEachQuestionIndividually() {
    ...
    for (let i = 0; i < questionsCount; i++) {
      try {
        ...
        if ((i + 1) % 10 === 0) {
          console.log(`📊 Прогрес: ${i + 1}/${questionsCount} питань, зібрано: ${this.questions.length} (${this.testConfig.title})`);
        }
    ...
    console.log(`✅ Індивідуальні кліки завершено. Зібрано ${this.questions.length} питань (${this.testConfig.title})`);
  ...
  async generateSQL(questionsData) {
    const sqlFilename = path.basename(questionsData).replace('.json', '.sql');
    const sqlFilepath = path.join(__dirname, 'data', sqlFilename);

    let sql = `-- SQL для імпорту питань фармації ${this.testConfig.title}\n`;
    sql += `-- Кількість питань: ${this.questions.length}\n`;
    ...
    this.questions.forEach((question, index) => {
      ...
      sql += `  '${this.testConfig.title}',\n`;
      sql += `  '${this.testConfig.identifier}',\n`;
      sql += `  ${questionImage},\n`;
      sql += `  ${optionImages},\n`;
  ...
  async run() {
    try {
      ...
      if (questions.length > 0) {
        ...
        const sqlFilepath = await this.generateSQL(jsonFilepath);
...
async function runAllScrapers() {
  console.log('🚀 ЗАПУСКАЄМО 6 СКРАПЕРІВ З ТАЙМЕРАМИ +45 СЕКУНД...\n');
  
  const results = [];
  
  for (let i = 0; i < testsConfig.length; i++) {
    const test = testsConfig[i];
    console.log(`\n🎯 СКРАПЕР ${i + 1}/6: ${test.title} (таймер: ${test.timerSeconds}с)`);
    
    const scraper = new PharmacyScraperWithTimer(test);
    ...
    if (result) {
      results.push({
        ...
        jsonFile: result.jsonFilepath,
        sqlFile: result.sqlFilepath,
        success: true
      });
      console.log(`✅ ${test.title}: ${result.questionsCount} питань`);
    } else {
      results.push({
        title: test.title,
        questionsCount: 0,
      ...
      console.log(`❌ ${test.title}: НЕ ВДАЛОСЯ ЗАВЕРШИТИ`);
    }
    
    // Невелика пауза між скраперами
    if (i < testsConfig.length - 1) {
      console.log(`\n⏳ Пауза 5 секунд перед наступним скрапером...`);
      await new Promise(resolve => setTimeout(resolve, 5000));
    }
  }
  
  console.log('\n🎉 ВСІ 6 СКРАПЕРІВ ЗАВЕРШЕНО!');
  console.log('\n📊 РЕЗУЛЬТАТИ:');
  results.forEach((result, index) => {
    if (result.success) {
      console.log(`${index + 1}. ${result.title}: ${result.questionsCount} питань ✅`);
      console.log(`   📄 JSON: ${result.jsonFile}`);
      console.log(`   📄 SQL: ${result.sqlFile}`);
    } else {
      console.log(`${index + 1}. ${result.title}: НЕ ВДАЛОСЯ ЗАВЕРШИТИ ❌`);
    }
  });
  
  const successfulCount = results.filter(r => r.success).length;
  console.log(`\n✅ Успішно завершено: ${successfulCount}/${results.length} скраперів`);
...
module.exports = { PharmacyScraperWithTimer, runAllScrapers };
