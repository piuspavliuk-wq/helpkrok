...
class MicrobiologyScraper {
  ...
  async init() {
    console.log('🚀 Запуск браузера для збору патоморфології...');
  ...
  generateSQL(questions) {
    console.log('📝 Генерація SQL для патоморфології...');
    
    let sql = '-- SQL для вставки питань з патоморфології\n';
    ...
    questions.forEach((question, index) => {
      ...
      sql += `INSERT INTO pathomorphology_questions (question_text, option_a, option_b, option_c, option_d, option_e, correct_answer, created_at) VALUES (\n`;
      sql += `  '${questionText}',\n`;
      sql += `  '${optionA}',\n`;
      sql += `  '${optionB}',\n`;
      sql += `  '${optionC}',\n`;
