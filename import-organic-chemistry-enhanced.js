...
async function importOrganicChemistryEnhanced() {
  try {
    console.log('🚀 Початок імпорту питань з органічної хімії (з підтримкою зображень)...');

    // Знаходимо останній JSON файл з enhanced images
    const dataDir = path.join(__dirname, 'data');
    const files = await fs.promises.readdir(dataDir);
    const jsonFiles = files.filter(file => file.startsWith('krok-lead-enhanced-images') && file.endsWith('.json'));
    
    if (jsonFiles.length === 0) {
      console.error('❌ Не знайдено жодного enhanced JSON файлу для імпорту.');
      return;
    }
    ...
    console.log(`🖼️ Питань з зображеннями: ${jsonData.questions_with_images || 0}`);
...
importOrganicChemistryEnhanced();
