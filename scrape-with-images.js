...
async function scrapeWithImages(url) {
  ...
  try {
    ...
    // Налаштування для кращого рендерингу зображень
    await page.setViewport({ width: 1920, height: 1080 });
    
    // Відключення завантаження зображень для швидкості, але збереження їх в DOM
    ...
    const filename = `krok-lead-with-images-${timestamp}.json`;
...
if (!url) {
  ...
  console.log('📝 Використання: node scrape-with-images.js <URL>');
  process.exit(1);
}

scrapeWithImages(url)
  .then((filepath) => {
