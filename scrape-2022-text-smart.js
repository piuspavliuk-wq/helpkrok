...
// Конфігурація для тесту "2022 (Текст)"
const testConfig = {
  year: 2022,
  identifier: '2022-pharmacy-text',
  title: '2022 (Текст)',
  timerSeconds: 30 // 30 секунд
};

class Pharmacy2022TextScraper {
...
async function run2022TextScraper() {
  console.log('🚀 ЗАПУСКАЄМО СКРАПЕР ДЛЯ "2022 (Текст)" З ТАЙМЕРОМ 30 СЕКУНД...');
  console.log('');
  
  const scraper = new Pharmacy2022TextScraper(testConfig);
...
if (require.main === module) {
  run2022TextScraper().catch(console.error);
}

module.exports = { Pharmacy2022TextScraper, run2022TextScraper };
