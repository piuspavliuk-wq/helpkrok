...
class AllVariantsScraper {
  ...
  async init() {
    console.log('🚀 Запуск браузера для збору ВСІХ варіантів відповідей...');
  ...
  async extractAllQuestionsWithAllVariants() {
    console.log('🔍 Збір ВСІХ питань з ВСІМА варіантами відповідей...');
      ...
      const questions = await this.page.evaluate(() => {
        ...
        questionElements.forEach((questionEl, index) => {
          try {
            ...
            // Спробуємо різні способи отримання варіантів
            const options = {};
            let correctAnswer = '';
            
            // Спосіб 1: Шукаємо в .question__variants
            const variantsContainer = questionEl.querySelector('.question__variants');
            if (variantsContainer) {
              const variantElements = variantsContainer.querySelectorAll('.question__variant');
              
              variantElements.forEach((variantEl, variantIndex) => {
                // Спробуємо різні селектори для тексту варіанту
                let optionText = '';
                
                // Спосіб 1: span всередині варіанту
                const spanEl = variantEl.querySelector('span');
                if (spanEl) {
                  optionText = cleanText(spanEl.textContent);
                }
                
                // Спосіб 2: весь текст варіанту
                if (!optionText) {
                  optionText = cleanText(variantEl.textContent);
                }
                
                // Спосіб 3: текст в дочірніх елементах
                if (!optionText) {
                  const textElements = variantEl.querySelectorAll('*');
                  for (const el of textElements) {
                    const text = cleanText(el.textContent);
                    if (text && text.length > 3 && !text.includes('Варіант')) {
                      optionText = text;
                      break;
                    }
                  }
                ...
                if (optionText && !optionText.includes('Варіант') && optionText.length > 3) {
                  ...
                  if (variantEl.classList.contains('question__variant_right') || 
                      ...
                      variantEl.querySelector('[class*="correct"]')) {
                    correctAnswer = letter;
                    console.log(`Правильна відповідь для питання ${index + 1}: ${letter} - ${optionText.substring(0, 50)}...`);
                  }
                }
              });
            }
            
            // Спосіб 2: Якщо не знайшли варіанти, спробуємо інші селектори
            if (Object.keys(options).length === 0) {
              const allVariants = questionEl.querySelectorAll('.question__variant');
              
              allVariants.forEach((variantEl, variantIndex) => {
                let optionText = '';
                
                // Спробуємо всі можливі способи отримання тексту
                const selectors = ['span', 'div', 'p', '.text', '.content', '*'];
                
                for (const selector of selectors) {
                  const element = variantEl.querySelector(selector);
                  if (element) {
                ...
                // Якщо все ще не знайшли, беремо весь текст елемента
                if (!optionText) {
                  optionText = cleanText(variantEl.textContent);
                }
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-all-variants-${Date.now()}.json`;
...
// Основна функція
async function scrapeAllVariants(url, timerSeconds = 10) {
  const scraper = new AllVariantsScraper();
  ...
  try {
    ...
    const questions = await scraper.extractAllQuestionsWithAllVariants();
...
if (require.main === module) {
  ...
  scrapeAllVariants(url, timerSeconds);
}

module.exports = { AllVariantsScraper, scrapeAllVariants };
