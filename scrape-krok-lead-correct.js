...
class CorrectKrokLeadScraper {
  ...
  async extractQuestionsWithCorrectAnswers() {
    console.log('🔍 Збір питань з правильними відповідями...');
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-correct-${Date.now()}.json`;
...
// Основна функція
async function scrapeCorrectKrokLead(url) {
  const scraper = new CorrectKrokLeadScraper();
  ...
  try {
    ...
    const questions = await scraper.extractQuestionsWithCorrectAnswers();
...
if (require.main === module) {
  ...
  scrapeCorrectKrokLead(url);
}

module.exports = { CorrectKrokLeadScraper, scrapeCorrectKrokLead };
