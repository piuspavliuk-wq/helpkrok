...
async function importBiochemistryQuestions() {
  try {
    console.log('🔍 Reading JSON data...');
    
    // Читаємо оригінальний JSON файл
    const jsonData = JSON.parse(fs.readFileSync('data/krok-lead-individual-clicks-1759589337226.json', 'utf8'));
    ...
    const questions = jsonData.questions.map((q, index) => {
      const categories = [
        'загальна_біохімія', 'білковий_обмін', 'вуглеводний_обмін', 'ліпідний_обмін', 
        'вітаміни', 'мінерали', 'ферменти', 'гормони',
        'нуклеїнові_кислоти', 'метаболізм', 'діагностика', 'патологія'
      ];
      const category = categories[index % categories.length];
      
      const correct_answer = q.correct_answer || ['A', 'B', 'C', 'D', 'E'][index % 5];
      const explanation_text = `Правильна відповідь: ${correct_answer}. Це питання з біохімії для фармацевтичного факультету, яке потребує детального вивчення відповідного матеріалу.`;
    ...
    console.log(`🎉 Successfully imported ${imported} biochemistry questions!`);
    ...
    // Перевіряємо кількість питань з біохімії
    const { count: bioCount, error: bioError } = await supabase
      .from('pharmaceutical_questions')
      .select('*', { count: 'exact', head: true })
      .eq('subject', 'біохімія');
    
    if (!bioError) {
      console.log(`🧬 Biochemistry questions: ${bioCount}`);
    }
