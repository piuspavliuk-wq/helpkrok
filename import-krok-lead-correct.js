...
class CorrectKrokLeadDataImporter {
  constructor() {
  ...
  // Конвертуємо дані в формат krok_questions_unified (БЕЗ difficulty, З правильними відповідями)
  convertToUnifiedFormat(scrapedData) {
    ...
    scrapedData.questions.forEach((question, index) => {
      try {
        ...
        // Використовуємо правильну відповідь з сайту
        ...
        const convertedQuestion = {
          ...
          explanation_text: this.generateExplanation(question.question_text, correctAnswer, options[correctAnswer]),
  ...
  // Визначаємо правильну відповідь (якщо не вказана)
  ...
  // Генеруємо пояснення з правильною відповіддю
  generateExplanation(questionText, correctAnswer, correctOptionText) {
    return `Правильна відповідь: ${correctAnswer}. ${correctOptionText || 'Це питання з медичної підготовки, яке потребує детального вивчення відповідного матеріалу.'}`;
  }

  // Створюємо SQL для імпорту (БЕЗ difficulty, З правильними відповідями)
  generateImportSQL(convertedQuestions) {
    let sql = `-- Імпорт питань з Krok Lead (з правильними відповідями)
...
`;
  ...
  // Створюємо JSON для API імпорту (БЕЗ difficulty, З правильними відповідями)
  generateAPIJSON(convertedQuestions) {
    return {
      source: 'krok-lead-correct-import',
  ...
  saveResults(convertedQuestions, originalData) {
    ...
    // SQL файл (БЕЗ difficulty, З правильними відповідями)
    const sqlContent = this.generateImportSQL(convertedQuestions);
    const sqlPath = path.join(__dirname, 'data', `krok-lead-correct-import-${timestamp}.sql`);
    fs.writeFileSync(sqlPath, sqlContent, 'utf8');
    
    // JSON файл для API (БЕЗ difficulty, З правильними відповідями)
    const jsonContent = this.generateAPIJSON(convertedQuestions);
    const jsonPath = path.join(__dirname, 'data', `krok-lead-correct-api-${timestamp}.json`);
    ...
    const report = {
      import_summary: {
        ...
        correct_answers_count: convertedQuestions.filter(q => q.correct_answer).length,
        improvements: [
          'Прибрано колонку difficulty',
          'Додано реальні варіанти відповідей',
          'Додано правильні відповіді з сайту',
          'Покращено пояснення з правильною відповіддю'
        ]
      },
      files_created: {
        sql_file: sqlPath,
      ...
      next_steps: [
        ...
        '5. Перевірити правильні відповіді'
      ]
    };
    
    const reportPath = path.join(__dirname, 'data', `krok-lead-correct-report-${timestamp}.json`);
    fs.writeFileSync(reportPath, JSON.stringify(report, null, 2), 'utf8');
    
    console.log('\n📊 Результати конвертації з правильними відповідями:');
    ...
    console.log(`- Правильних відповідей: ${report.import_summary.correct_answers_count}`);
...
// Основна функція
async function importCorrectKrokLeadData(dataFilePath) {
  const importer = new CorrectKrokLeadDataImporter();
  ...
  try {
    ...
    // Конвертуємо в формат krok_questions_unified (БЕЗ difficulty, З правильними відповідями)
    console.log('\n🔄 Конвертація даних з правильними відповідями...');
    ...
    // Зберігаємо результати
    console.log('\n💾 Збереження результатів з правильними відповідями...');
    const results = importer.saveResults(convertedQuestions, scrapedData);
    
    console.log('\n✅ Конвертація з правильними відповідями завершена успішно!');
    console.log('\n📋 Наступні кроки:');
    ...
    console.log('5. Перевірте правильні відповіді');
    
  } catch (error) {
    console.error('❌ Помилка при імпорті з правильними відповідями:', error);
  }
}
...
if (require.main === module) {
  ...
  if (!dataFilePath) {
    ...
    console.log('Використання: node import-krok-lead-correct.js <path-to-data-file>');
    process.exit(1);
  }
  
  importCorrectKrokLeadData(dataFilePath);
}

module.exports = { CorrectKrokLeadDataImporter, importCorrectKrokLeadData };
