...
async function importPathophysiologyQuestions() {
  try {
    console.log('🚀 Початок імпорту питань з патофізіології...');
    ...
    console.log(`✅ Успішно імпортовано ${questionsToInsert.length} питань з патофізіології`);

    // Перевіряємо загальну кількість питань в базі
    const { count, error: countError } = await supabase
      .from('pharmaceutical_questions')
      .select('*', { count: 'exact' })
      .eq('subject', 'патофізіологія');
    ...
    console.log(`📊 Загальна кількість питань з патофізіології в базі: ${count}`);
...
importPathophysiologyQuestions();
