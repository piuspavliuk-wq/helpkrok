...
// Конфігурація тестів з збільшеними таймерами
const tests = [
  {
    year: 2019,
...
class UniversalPharmacyScraper {
...
async function runAllScrapersParallel() {
  console.log('🚀 ЗАПУСКАЄМО ВСІ 7 СКРАПЕРІВ ОДНОЧАСНО З ЗБІЛЬШЕНИМИ ТАЙМЕРАМИ...');
  console.log('');
  ...
  const promises = tests.map((test, index) => {
    ...
    const scraper = new UniversalPharmacyScraper(test);
    return scraper.run();
  });
  
  console.log('\n⏰ ВСІ СКРАПЕРИ ЗАПУЩЕНО! Чекаємо завершення...\n');
...
module.exports = { UniversalPharmacyScraper, runAllScrapersParallel };
