...
class OrganicChemistrySmartV2Scraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Запуск браузера для розумного збору органічної хімії v2...');
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-organic-chemistry-smart-v2-${Date.now()}.json`;
...
async function scrapeOrganicChemistrySmartV2(url, timerSeconds = 30) {
  const scraper = new OrganicChemistrySmartV2Scraper();
...
if (require.main === module) {
  ...
  scrapeOrganicChemistrySmartV2(url, timerSeconds);
}

module.exports = { OrganicChemistrySmartV2Scraper, scrapeOrganicChemistrySmartV2 };
