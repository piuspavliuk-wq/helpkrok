...
// Конфігурація для тесту "2025 рік (ТЕХТ)"
const testConfig = {
  year: 2025,
  identifier: '2025-pharmacy-text',
  title: '2025 рік (ТЕХТ)',
  timerSeconds: 30 // 30 секунд
};

class Pharmacy2025TextScraper {
      ...
      try {
        ...
        const questionData = await this.page.evaluate((index) => {
          ...
          if (questions[index]) {
            ...
            // Отримуємо текст зверху (основний текст)
            let mainText = '';
            const mainTextEl = document.querySelector('.test-content__text, .test-text, .main-text, .text-content');
            if (mainTextEl) {
              mainText = cleanText(mainTextEl.textContent);
            }
            ...
            // Об'єднуємо основний текст з текстом питання
            const fullQuestionText = mainText ? `${mainText}\n\n${questionText}` : questionText;
...
async function run2025TextScraper() {
  console.log('🚀 ЗАПУСКАЄМО СКРАПЕР ДЛЯ "2025 рік (ТЕХТ)" З ТАЙМЕРОМ 30 СЕКУНД...');
  console.log('');
  
  const scraper = new Pharmacy2025TextScraper(testConfig);
...
if (require.main === module) {
  run2025TextScraper().catch(console.error);
}

module.exports = { Pharmacy2025TextScraper, run2025TextScraper };
