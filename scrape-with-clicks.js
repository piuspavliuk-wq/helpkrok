...
class ClickKrokLeadScraper {
  ...
  async init() {
    console.log('🚀 Запуск браузера з кліками на кожне питання...');
  ...
  async clickOnEachQuestion() {
    console.log('🖱️ Клікаємо на кожне питання для розкриття варіантів...');
    ...
    for (let i = 0; i < questionsCount; i++) {
      try {
        ...
        await this.page.evaluate((index) => {
          const questions = document.querySelectorAll('.question');
          if (questions[index]) {
            // Спробуємо клікнути на різні елементи питання
        ...
        // Перевіряємо, чи завантажилися варіанти
        const variantsLoaded = await this.page.evaluate((index) => {
          const questions = document.querySelectorAll('.question');
          if (questions[index]) {
            const variants = questions[index].querySelectorAll('.question__variant span');
        ...
        if (variantsLoaded) {
          console.log(`✅ Питання ${i + 1}: варіанти завантажені`);
        } else {
          console.log(`⚠️ Питання ${i + 1}: варіанти не завантажені`);
        }
        
        // Показуємо прогрес кожні 10 питань
        if ((i + 1) % 10 === 0) {
          console.log(`📊 Прогрес: ${i + 1}/${questionsCount} питань`);
        }
        
      } catch (error) {
        console.log(`❌ Помилка при кліку на питання ${i + 1}:`, error.message);
      }
    }
    
    console.log('✅ Кліки на всі питання завершено');
  }
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-clicks-${Date.now()}.json`;
...
// Основна функція
async function scrapeWithClicks(url, timerSeconds = 10) {
  const scraper = new ClickKrokLeadScraper();
  ...
  try {
    ...
    // Клікаємо на кожне питання
    await scraper.clickOnEachQuestion();
...
if (require.main === module) {
  ...
  scrapeWithClicks(url, timerSeconds);
}

module.exports = { ClickKrokLeadScraper, scrapeWithClicks };
