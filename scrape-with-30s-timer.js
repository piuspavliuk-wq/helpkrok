...
async function scrapeWith30sTimer(url) {
  console.log('🚀 Запуск браузера з 30-секундним таймером...');
  ...
  try {
    ...
    const filename = `krok-lead-30s-${timestamp}.json`;
...
if (!url) {
  ...
  console.log('📝 Використання: node scrape-with-30s-timer.js <URL>');
  process.exit(1);
}

scrapeWith30sTimer(url)
  .then((filepath) => {
