...
// Конфігурація для тесту "2024р 2д"
const testConfig = {
  year: 2024,
  identifier: '2024-pharmacy-2d',
  title: '2024р 2д',
  timerSeconds: 30 // 30 секунд
};

class Pharmacy2024Day2Scraper {
  ...
  async scrollToLoadAllQuestions() {
    ...
    const maxScrollAttempts = 20; // Збільшуємо для більшого тесту
...
async function run2024Day2Scraper() {
  console.log('🚀 ЗАПУСКАЄМО СКРАПЕР ДЛЯ "2024р 2д" З ТАЙМЕРОМ 30 СЕКУНД...');
  console.log('');
  
  const scraper = new Pharmacy2024Day2Scraper(testConfig);
...
if (require.main === module) {
  run2024Day2Scraper().catch(console.error);
}

module.exports = { Pharmacy2024Day2Scraper, run2024Day2Scraper };
