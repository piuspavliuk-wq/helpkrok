...
class OrganicChemistryWithImagesScraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Запуск браузера для органічної хімії з підтримкою зображень...');
      ...
      try {
        ...
        const questionData = await this.page.evaluate((index) => {
          ...
          if (questions[index]) {
            ...
            // Перевіряємо, чи є зображення в питанні
            const questionImageEl = questionEl.querySelector('.question__text img');
            let questionImage = null;
            if (questionImageEl) {
              questionImage = getImageAsBase64(questionImageEl);
            }
              ...
              if (imgEl) {
                ...
                if (svgEl) {
                  ...
                  if (backgroundImage && backgroundImage !== 'none') {
                    // Для background-image ми не можемо легко отримати base64
                    // Але можемо зберегти URL
        ...
        if (questionData) {
          ...
          console.log(`✅ Питання ${i + 1}: зібрано ${Object.keys(questionData.options).length} варіантів, правильна: ${questionData.correct_answer || 'не визначено'}, зображень: ${imagesCount}`);
        } else {
  ...
  async saveQuestions(questions, pageInfo) {
    const data = {
      ...
      questions_with_images: questions.filter(q => q.question_image || Object.keys(q.option_images || {}).length > 0).length
    };

    const filename = `krok-lead-organic-chemistry-with-images-${Date.now()}.json`;
...
async function scrapeOrganicChemistryWithImages(url, timerSeconds = 10) {
  const scraper = new OrganicChemistryWithImagesScraper();
  ...
  try {
    ...
    if (questions.length > 0) {
      ...
      const withImagesCount = questions.filter(q => q.question_image || Object.keys(q.option_images || {}).length > 0).length;
...
if (require.main === module) {
  ...
  scrapeOrganicChemistryWithImages(url, timerSeconds);
}

module.exports = { OrganicChemistryWithImagesScraper, scrapeOrganicChemistryWithImages };
