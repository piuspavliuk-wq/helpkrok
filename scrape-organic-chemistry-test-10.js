...
class OrganicChemistryTestScraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Запуск браузера для тестового збору органічної хімії (10 питань)...');
    ...
    for (let i = 0; i < maxQuestions; i++) {
      try {
        ...
        if (questionData) {
          ...
          console.log(`✅ Питання ${i + 1}: зібрано ${Object.keys(questionData.options).length} варіантів, правильна: ${questionData.correct_answer || 'не визначено'}, тип: ${questionType}, зображень: ${imagesCount}`);
        } else {
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-organic-chemistry-test-10-${Date.now()}.json`;
...
async function scrapeOrganicChemistryTest10(url, timerSeconds = 10) {
  const scraper = new OrganicChemistryTestScraper();
...
if (require.main === module) {
  ...
  scrapeOrganicChemistryTest10(url, timerSeconds);
}

module.exports = { OrganicChemistryTestScraper, scrapeOrganicChemistryTest10 };
