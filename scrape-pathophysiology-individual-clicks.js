...
async function scrapePathophysiologyQuestions(baseUrl, waitTime = 30) {
  console.log('🚀 Запуск браузера для збору патфізіології...');
  
  const browser = await puppeteer.launch({
    headless: false, // Показуємо браузер для ручного налаштування
  ...
  try {
    ...
    while (scrollAttempts < maxScrollAttempts && currentHeight > previousHeight) {
      ...
      const questionCount = await page.$$eval('.question', questions => questions.length);
    ...
    const jsonFilename = `data/krok-lead-pathophysiology-individual-clicks-${timestamp}.json`;
    fs.writeFileSync(jsonFilename, JSON.stringify(result, null, 2));
    console.log(`💾 Дані збережено в файл: ${path.resolve(jsonFilename)}`);

    // Генеруємо SQL
    console.log('📝 Генерація SQL для патфізіології...');
    const sqlFilename = `data/pathophysiology-questions-${timestamp}.sql`;
    let sqlContent = `-- SQL для вставки питань з патфізіології
...
`;
    ...
    collectedQuestions.forEach(question => {
      ...
      sqlContent += `INSERT INTO pathophysiology_questions (question_text, option_a, option_b, option_c, option_d, option_e, correct_answer, created_at) VALUES (
  '${escapedQuestion}',
  '${escapedA}',
  '${escapedB}',
  '${escapedC}',
...
scrapePathophysiologyQuestions(baseUrl, waitTime);
