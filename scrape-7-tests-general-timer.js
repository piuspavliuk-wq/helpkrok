...
const tests = [
  {
    year: 2019,
    identifier: '2019-pharmacy-eng',
    title: '2019 (Eng)'
  },
  {
    year: 2022,
    identifier: '2022-pharmacy',
    title: '2022 рік'
  },
  {
    year: 2023,
    identifier: '2023-pharmacy-distance',
    title: '2023 заочне'
  },
  {
    year: 2023,
    identifier: '2023-pharmacy-full-time',
    title: '2023 очне'
  },
  {
    year: 2024,
    identifier: '2024-pharmacy-new-ct',
    title: 'Нові тести з цт 2024'
  },
  {
    year: 2024,
    identifier: '2024-pharmacy-bulgaria',
    title: '2024 Болгарія'
  },
  {
    year: 2024,
    identifier: '2024-pharmacy-1d',
    title: '2024р 1д'
  }
];

class GeneralTimerPharmacyScraper {
...
async function runAllScrapersWithGeneralTimer() {
  console.log('🚀 ЗАПУСКАЄМО 7 СКРАПЕРІВ З ЗАГАЛЬНИМ ТАЙМЕРОМ 3 ХВИЛИНИ...');
  console.log('');
  
  // Запускаємо всі скрапери одночасно
  const promises = tests.map((test, index) => {
    console.log(`🎯 Запускаємо скрапер ${index + 1}/7: ${test.title}`);
    const scraper = new GeneralTimerPharmacyScraper(test);
    return scraper.run();
  });
  
  console.log('\n⏰ ВСІ 7 СКРАПЕРІВ ЗАПУЩЕНО!');
  console.log('📋 ЗАГАЛЬНИЙ ТАЙМЕР: 3 ХВИЛИНИ ДЛЯ НАЛАШТУВАННЯ ВСІХ ТЕСТІВ');
  ...
  console.log('5. Повторіть для всіх 7 браузерів');
  console.log('6. Чекайте завершення загального таймера...\n');
  
  // Загальний таймер 3 хвилини
  console.log('⏰ ЗАГАЛЬНИЙ ТАЙМЕР 3 ХВИЛИНИ ПОЧАТО...');
  for (let i = 180; i > 0; i--) {
    const minutes = Math.floor(i / 60);
    const seconds = i % 60;
    console.log(`⏳ Залишилось: ${minutes}:${seconds.toString().padStart(2, '0')} (${Math.round((180-i)/180*100)}%)`);
    await new Promise(resolve => setTimeout(resolve, 1000));
  }
  
  console.log('\n🚀 ЗАГАЛЬНИЙ ТАЙМЕР ЗАВЕРШЕНО! Продовжуємо збір даних...\n');
  
  try {
    const results = await Promise.all(promises);
    
    console.log('\n🎉 ВСІ 7 СКРАПЕРІВ ЗАВЕРШЕНО!');
    console.log('\n📊 РЕЗУЛЬТАТИ:');
    
    results.forEach((result, index) => {
      if (result) {
        console.log(`${index + 1}. ${tests[index].title}: ${result.questionsCount} питань ✅`);
        console.log(`   📄 JSON: ${result.jsonFilepath}`);
        console.log(`   📄 SQL: ${result.sqlFilepath}`);
        console.log('');
      } else {
        console.log(`${index + 1}. ${tests[index].title}: НЕ ВДАЛОСЯ ЗАВЕРШИТИ ❌`);
        console.log('');
      }
    });
    
    const successful = results.filter(r => r !== null).length;
    console.log(`\n✅ Успішно завершено: ${successful}/7 скраперів`);
...
if (require.main === module) {
  runAllScrapersWithGeneralTimer().catch(console.error);
}

module.exports = { GeneralTimerPharmacyScraper, runAllScrapersWithGeneralTimer };
