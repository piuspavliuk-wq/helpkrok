...
class IndividualClicksScraperWithImages {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Запуск браузера з індивідуальними кліками та підтримкою зображень...');
  ...
  async clickOnEachQuestionIndividually() {
    ...
    for (let i = 0; i < questionsCount; i++) {
      try {
        ...
        // Збираємо дані з поточного питання (включаючи зображення)
        const questionData = await this.page.evaluate(async (index) => {
          const questions = document.querySelectorAll('.question');
          if (questions[index]) {
            const questionEl = questions[index];
            ...
            // Отримуємо текст питання з зображеннями
            ...
            if (questionTextEl) {
              ...
              questionImages = await convertImagesToBase64(questionTextEl);
            }
            
            if (!questionText && questionImages.length === 0) {
              return null;
            }
            
            // Отримуємо варіанти відповідей з зображеннями
            ...
            for (let variantIndex = 0; variantIndex < variantElements.length; variantIndex++) {
              ...
              // Конвертуємо зображення в варіантах
              const optionImages = await convertImagesToBase64(variantEl);
              
              if ((optionText && !optionText.includes('Варіант') && optionText.length > 5) || optionImages.length > 0) {
        ...
        if (questionData) {
          ...
          const imageCount = questionData.question_images.length + 
                           Object.values(questionData.options).reduce((sum, opt) => sum + opt.images.length, 0);
          const imageStatus = imageCount > 0 ? ` (${imageCount} зображень)` : '';
          console.log(`✅ Питання ${i + 1}: зібрано ${Object.keys(questionData.options).length} варіантів, правильна: ${questionData.correct_answer || 'не визначено'}${imageStatus}`);
        } else {
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-individual-clicks-images-${Date.now()}.json`;
...
// Основна функція
async function scrapeWithIndividualClicksAndImages(url, timerSeconds = 10) {
  const scraper = new IndividualClicksScraperWithImages();
    ...
    if (questions.length > 0) {
      ...
      const totalImages = questions.reduce((sum, q) => {
        return sum + q.question_images.length + 
               Object.values(q.options).reduce((optSum, opt) => optSum + opt.images.length, 0);
      }, 0);
      ...
      questions.slice(0, 3).forEach((q, i) => {
        ...
        console.log(`   Зображення в питанні: ${q.question_images.length}`);
        Object.entries(q.options).forEach(([letter, opt]) => {
          const isCorrect = letter === q.correct_answer ? ' ✓' : '';
          const imageCount = opt.images.length;
          const imageStatus = imageCount > 0 ? ` (${imageCount} зображень)` : '';
          console.log(`   ${letter}: ${opt.text.substring(0, 50)}...${imageStatus}${isCorrect}`);
        });
      });
...
if (require.main === module) {
  const url = process.argv[2] || 'https://krok-lead.com/base/159be196-c4f8-4215-b0b5-cb598cffd8de?faculty=pharmaceutical&krok=krok1';
  const timerSeconds = parseInt(process.argv[3]) || 10;
  scrapeWithIndividualClicksAndImages(url, timerSeconds);
}

module.exports = { IndividualClicksScraperWithImages, scrapeWithIndividualClicksAndImages };
