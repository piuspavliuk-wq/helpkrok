...
async function scrapeWithImagesBase64(url) {
  console.log('🚀 Запуск браузера з збереженням зображень в base64...');
  ...
  try {
    ...
    const filename = `krok-lead-images-${timestamp}.json`;
...
if (!url) {
  ...
  console.log('📝 Використання: node scrape-with-images-base64.js <URL>');
  process.exit(1);
}

scrapeWithImagesBase64(url)
  .then((filepath) => {
