...
SELECT c.id, 'Патологічні процеси', 'Імпорт із Untitled document.txt (патологічні процеси)', 1, true, NOW(), NOW()
FROM courses c
WHERE c.title = 'Система кровотворення й імунного захисту, кров'
  AND NOT EXISTS (SELECT 1 FROM topics t WHERE t.course_id = c.id AND t.title = 'Патологічні процеси');
