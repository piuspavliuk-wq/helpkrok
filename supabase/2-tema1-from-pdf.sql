...
WITH inserted_question AS (
  INSERT INTO questions (topic_id, question_text, explanation, difficulty, order_index, is_active, image_url)
  SELECT t.id, 'полука відноситься до похідних ароматичних вуглеводнів, не утворює забарвлення з FeCl3 , при окисненні утворює бензойну кислоту. Це: o', NULL, 'medium', 9, true, '/test-images/organic/tema1/page2_img1.png'
  FROM topics t
  JOIN courses c ON c.id = t.course_id
  WHERE c.title = 'Основи знань про органічні сполуки'
    AND t.title = 'Вуглеводні та їх галогенопохідні'
...
WITH inserted_question AS (
  INSERT INTO questions (topic_id, question_text, explanation, difficulty, order_index, is_active, image_url)
  SELECT t.id, 'Які продукти реакції, що утворюються за механізмом радикального заміщення вказані на нижченаведеній схемі перетворень? o', NULL, 'medium', 48, true, '/test-images/organic/tema1/page13_img1.png'
  FROM topics t
  JOIN courses c ON c.id = t.course_id
  WHERE c.title = 'Основи знань про органічні сполуки'
    AND t.title = 'Вуглеводні та їх галогенопохідні'
