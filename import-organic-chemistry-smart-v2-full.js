...
async function importOrganicChemistrySmartV2Full() {
  try {
    console.log('🚀 Початок імпорту питань з органічної хімії (повна версія з зображеннями)...');
    ...
    // Знаходимо найновіший JSON файл v2
    const dataDir = path.join(__dirname, 'data');
    const files = await fs.promises.readdir(dataDir);
    const jsonFiles = files.filter(file => 
      file.startsWith('krok-lead-organic-chemistry-smart-v2-') && 
      file.endsWith('.json')
    );
    
    if (jsonFiles.length === 0) {
      console.error('❌ Не знайдено жодного JSON файлу v2 для імпорту.');
      return;
    }
    ...
    // Підготовка даних для вставки з підтримкою зображень
    const questionsToInsert = questions.map(q => {
      ...
      if (q.question_image) {
        baseData.question_image = q.question_image;
      }
      
      if (q.option_images) {
        if (q.option_images.A) baseData.option_a_image = q.option_images.A;
        if (q.option_images.B) baseData.option_b_image = q.option_images.B;
        if (q.option_images.C) baseData.option_c_image = q.option_images.C;
        if (q.option_images.D) baseData.option_d_image = q.option_images.D;
        if (q.option_images.E) baseData.option_e_image = q.option_images.E;
      }

      return baseData;
    });

    console.log('📝 Підготовлено дані для вставки з підтримкою зображень');
    ...
    const withImagesCount = questionsToInsert.filter(q => 
      ...
      q.option_c_image || q.option_d_image || q.option_e_image
    ).length;
    const textOnlyCount = questionsToInsert.length - withImagesCount;
    ...
    console.log('🎉 Повний імпорт з підтримкою зображень завершено успішно!');
...
importOrganicChemistrySmartV2Full();
