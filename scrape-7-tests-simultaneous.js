...
const { Pharmacy2018UkrEngScraper } = require('./scrape-2018-pharmacy-ukr-eng.js');
...
async function runAllScrapers() {
  console.log('🚀 ЗАПУСКАЄМО ВСІ 7 СКРАПЕРІВ ОДНОЧАСНО...');
  console.log('');
  ...
  for (let i = 0; i < tests.length; i++) {
    const test = tests[i];
    console.log(`\n🎯 Запускаємо скрапер ${i + 1}/7: ${test.title}`);
    ...
    if (result) {
      ...
      console.log(`✅ ${test.title} завершено: ${result.questionsCount} питань`);
    } else {
  ...
  results.forEach((result, index) => {
    console.log(`${index + 1}. ${result.test}: ${result.questionsCount} питань`);
...
module.exports = { UniversalPharmacyScraper, runAllScrapers };
