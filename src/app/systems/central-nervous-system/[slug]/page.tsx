...
export default function SectionOrTopicPage({ params }: PageProps) {
  ...
  useEffect(() => {
    if (!section && !topic) {
      router.push('/systems/central-nervous-system')
    }
  ...
  if (section) {
    ...
    // Перший розділ або розділ з доступом - показуємо контент
    if (isFirstSection || canAccess) {
      return (
        <AuthGuard>
          <div className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-50 to-blue-100 relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="px-[3px] py-8 md:px-6 sm:px-8 md:px-12">
                <Link
                  href="/systems/central-nervous-system"
              ...
              <SectionContent 
                section={section} 
                courseTitle="Центральна нервова система (ЦНС) і периферична нервова система (ПНС). Органи чуття"
                faculty="medical"
              />
            </div>
          </div>
    ...
    // Якщо немає доступу і не завантажується
    if (!canAccess && !checkingAccess && !loading) {
      return (
        <AuthGuard>
          <div className="min-h-screen bg-gradient-to-br from-blue-50 via-blue-50 to-blue-100 relative overflow-hidden">
            <div className="relative z-10">
