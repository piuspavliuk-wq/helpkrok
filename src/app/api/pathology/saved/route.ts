...
// Отримати збережені питання з патології користувача
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: savedQuestions, error: savedError } = await supabase
      ...
      .eq('question_type', 'pathology')
      .order('saved_at', { ascending: false });

    if (savedError) {
      console.error('Помилка отримання збережених питань з патології:', savedError);
...
// Зберегти питання з патології користувача
