// API endpoint для збережених питань з біології
// Файл: /api/biology/saved/route.ts
...
// Отримати збережені питання з біології користувача
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: savedQuestions, error: savedError } = await supabase
      ...
      .eq('question_type', 'biology')
      .order('saved_at', { ascending: false });

    if (savedError) {
      console.error('Помилка отримання збережених питань з біології:', savedError);
...
// Зберегти питання з біології користувача
