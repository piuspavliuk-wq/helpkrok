...
// Отримати збережені питання з мікробіології користувача
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: savedQuestions, error: savedError } = await supabase
      ...
      .eq('question_type', 'microbiology')
      .order('saved_at', { ascending: false });

    if (savedError) {
      console.error('Помилка отримання збережених питань з мікробіології:', savedError);
...
// Зберегти питання з мікробіології користувача
