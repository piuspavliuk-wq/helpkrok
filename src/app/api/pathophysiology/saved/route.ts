...
// Отримати збережені питання з патологічної фізіології користувача
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: savedQuestions, error: savedError } = await supabase
      ...
      .eq('question_type', 'pathophysiology')
      .order('saved_at', { ascending: false });

    if (savedError) {
      console.error('Помилка отримання збережених питань з патологічної фізіології:', savedError);
...
// Зберегти питання з патологічної фізіології користувача
