...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log('Fetching anatomy questions with IDs:', questionIds);
    ...
    console.log(`Found ${questions?.length || 0} anatomy questions`);
    
    return NextResponse.json({ questions: questions || [] });
  } catch (error) {
    console.error('Error in /api/anatomy/questions-by-ids:', error);
