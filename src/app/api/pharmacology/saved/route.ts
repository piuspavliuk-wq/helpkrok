// API endpoint для збережених питань з фармакології
// Файл: /api/pharmacology/saved/route.ts
...
// Отримати збережені питання з фармакології користувача
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: savedQuestions, error: savedError } = await supabase
      ...
      .eq('question_type', 'pharmacology')
      .order('saved_at', { ascending: false });

    if (savedError) {
      console.error('Помилка отримання збережених питань з фармакології:', savedError);
...
// Зберегти питання з фармакології користувача
