// API для прогресу користувача з реальною базою даних
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Якщо немає прогресу, повертаємо 0
    if (!progress || progress.length === 0) {
      return NextResponse.json({
        ...
        progress: [{
          id: 1,
          user_id: userId,
          year: parseInt(year || '2014'),
          faculty: faculty,
          completed_questions: 0,
          total_questions: 5, // Загальна кількість питань
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Логуємо прогрес
    console.log(`Прогрес користувача ${userId}: ${completedQuestions}/${totalQuestions} для КРОК ${year} ${faculty}`);

    return NextResponse.json({
      success: true,
      message: 'Прогрес збережено',
      progress: result
    });
