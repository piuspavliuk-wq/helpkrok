...
async function importOrganicChemistryTest10() {
  try {
    console.log('🚀 Початок тестового імпорту питань з органічної хімії (10 питань)...');
    ...
    // Знаходимо найновіший тестовий JSON файл
    const dataDir = path.join(__dirname, 'data');
    const files = await fs.promises.readdir(dataDir);
    const jsonFiles = files.filter(file => 
      file.startsWith('krok-lead-organic-chemistry-test-10-') && 
      file.endsWith('.json')
    );
    
    if (jsonFiles.length === 0) {
      console.error('❌ Не знайдено жодного тестового JSON файлу для імпорту.');
      return;
    }
    ...
    console.log('🎉 Тестовий імпорт завершено успішно!');
