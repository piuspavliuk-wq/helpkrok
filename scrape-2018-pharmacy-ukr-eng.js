...
class Pharmacy2018UkrEngScraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Ініціалізація скрапера для 2018 рік Укр+Eng...');
  ...
  async saveQuestions(questions) {
    const data = {
      ...
      year: 2018,
      faculty: 'pharmacy',
      test_identifier: '2018-pharmacy-ukr-eng',
      title: '2018 рік Укр+Eng',
      questions: questions,
    ...
    const filename = `2018-pharmacy-ukr-eng-questions-${Date.now()}.json`;
  ...
  async generateSQL(questions, jsonFilepath) {
    ...
    let sql = `-- SQL для імпорту питань фармації 2018 рік Укр+Eng\n`;
    sql += `-- Кількість питань: ${questions.length}\n`;
    sql += `-- Назва тесту: 2018 рік Укр+Eng\n\n`;
    ...
    questions.forEach((question, index) => {
      ...
      sql += `  '2018 рік Укр+Eng',\n`;
      sql += `  '2018-pharmacy-ukr-eng',\n`;
      sql += `  ${questionImage},\n`;
      sql += `  ${optionImages},\n`;
  ...
  async run() {
    try {
      ...
      if (questions.length > 0) {
        ...
        console.log(`🎉 Успішно зібрано дані для 2018 рік Укр+Eng!`);
      ...
      console.error(`❌ Помилка при зборі даних для 2018 рік Укр+Eng:`, error);
      return null;
    } finally {
      await this.close();
    }
...
async function scrape2018PharmacyUkrEng() {
  const scraper = new Pharmacy2018UkrEngScraper();
  return await scraper.run();
}

if (require.main === module) {
  scrape2018PharmacyUkrEng().catch(console.error);
}

module.exports = { Pharmacy2018UkrEngScraper, scrape2018PharmacyUkrEng };
