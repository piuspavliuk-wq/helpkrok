...
async function scrapeAdvanced(url) {
  ...
  try {
    ...
    const jsonFilename = `2025-day1-eng-questions-${timestamp}.json`;
...
async function main() {
  console.log('🎯 РОЗШИРЕНИЙ СКРАПЕР для тесту "2025р 1 д (Eng)"');
  console.log('📋 Параметри:');
  console.log('   - Рік: 2025');
  console.log('   - Факультет: medical');
  console.log('   - Test ID: 2025-day1-eng');
  console.log('   - Назва: 2025р 1 д (Eng)');
  console.log('');
  ...
  if (questions.length > 0) {
    ...
    // Генеруємо SQL
    const timestamp = Date.now();
    const sqlFilename = `2025-day1-eng-questions-${timestamp}.sql`;
    const scraper = new AdvancedKrokScraper();
    await scraper.generateSQL(questions, sqlFilename, 2025, 'medical', '2025-day1-eng', '2025р 1 д (Eng)');
