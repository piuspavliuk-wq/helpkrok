...
class FixedOrganicChemistryScraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Запуск браузера для органічної хімії з виправленою обробкою зображень...');
        ...
        const questionData = await this.page.evaluate((index) => {
          ...
          if (questions[index]) {
            ...
            for (let variantIndex = 0; variantIndex < variantElements.length; variantIndex++) {
              ...
              // Якщо немає тексту або текст короткий, шукаємо зображення
              if (!optionText || optionText.length < 5) {
                const imgEl = variantEl.querySelector('img');
                if (imgEl) {
                  // Якщо є зображення, просто ставимо букву
                  optionText = letter;
                } else {
                  ...
                  if (svgEl) {
                    ...
                    if (backgroundImage && backgroundImage !== 'none') {
                      ...
                      // Якщо немає контенту, просто ставимо букву
                      optionText = letter;
                    }
                  }
                }
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-organic-chemistry-fixed-${Date.now()}.json`;
...
// Основна функція
async function scrapeOrganicChemistryFixed(url, timerSeconds = 10) {
  const scraper = new FixedOrganicChemistryScraper();
    ...
    if (questions.length > 0) {
      ...
      questions.slice(0, 5).forEach((q, i) => {
        ...
        Object.entries(q.options).forEach(([letter, text]) => {
          ...
          console.log(`   ${letter}: ${text}${isCorrect}`);
        });
      });
...
if (require.main === module) {
  ...
  scrapeOrganicChemistryFixed(url, timerSeconds);
}

module.exports = { FixedOrganicChemistryScraper, scrapeOrganicChemistryFixed };
