...
class ScrollKrokLeadScraper {
  ...
  async init() {
    console.log('🚀 Запуск браузера з прокруткою до кожного питання...');
  ...
  async scrollToEachQuestion() {
    console.log('📜 Прокручуємо до кожного питання для завантаження варіантів...');
    ...
    for (let i = 0; i < questionsCount; i++) {
      try {
        ...
        console.log(`❌ Помилка при прокрутці до питання ${i + 1}:`, error.message);
      }
    }
    
    console.log('✅ Прокрутка до всіх питань завершена');
  }
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-scroll-${Date.now()}.json`;
...
// Основна функція
async function scrapeWithScroll(url, timerSeconds = 10) {
  const scraper = new ScrollKrokLeadScraper();
  ...
  try {
    ...
    // Прокручуємо до кожного питання
    await scraper.scrollToEachQuestion();
...
if (require.main === module) {
  ...
  scrapeWithScroll(url, timerSeconds);
}

module.exports = { ScrollKrokLeadScraper, scrapeWithScroll };
