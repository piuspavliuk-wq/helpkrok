...
class OrganicChemistrySmartScraper {
  constructor() {
    this.browser = null;
    this.page = null;
  }
  ...
  async init() {
    console.log('🚀 Запуск браузера для розумного збору органічної хімії...');
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-organic-chemistry-smart-${Date.now()}.json`;
...
async function scrapeOrganicChemistrySmart(url, timerSeconds = 30) {
  const scraper = new OrganicChemistrySmartScraper();
    ...
    if (questions.length > 0) {
      ...
      questions.slice(0, 5).forEach((q, i) => {
        ...
        Object.entries(q.options).forEach(([letter, text]) => {
          ...
          console.log(`   ${letter}: ${text.substring(0, 50)}...${isCorrect}${hasImage}`);
        });
      });
...
if (require.main === module) {
  ...
  scrapeOrganicChemistrySmart(url, timerSeconds);
}

module.exports = { OrganicChemistrySmartScraper, scrapeOrganicChemistrySmart };
