...
// Конфігурація для тесту "2024 рік (TEXT)"
const testConfig = {
  year: 2024,
  identifier: '2024-pharmacy-text',
  title: '2024 рік (TEXT)',
  timerSeconds: 30 // 30 секунд
};

class Pharmacy2024TextScraper {
...
async function run2024TextScraper() {
  console.log('🚀 ЗАПУСКАЄМО СКРАПЕР ДЛЯ "2024 рік (TEXT)" З ТАЙМЕРОМ 30 СЕКУНД...');
  console.log('');
  
  const scraper = new Pharmacy2024TextScraper(testConfig);
...
if (require.main === module) {
  run2024TextScraper().catch(console.error);
}

module.exports = { Pharmacy2024TextScraper, run2024TextScraper };
