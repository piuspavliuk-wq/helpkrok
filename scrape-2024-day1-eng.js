...
async function scrapeAdvanced(url) {
  ...
  try {
    ...
    const jsonFilename = `2024-day1-eng-questions-${timestamp}.json`;
...
async function main() {
  console.log('🎯 РОЗШИРЕНИЙ СКРАПЕР для тесту "2024р 1 день (Eng)"');
  console.log('📋 Параметри:');
  console.log('   - Рік: 2024');
  console.log('   - Факультет: medical');
  console.log('   - Test ID: 2024-day1-eng');
  console.log('   - Назва: 2024р 1 день (Eng)');
  console.log('');
  ...
  if (questions.length > 0) {
    ...
    // Генеруємо SQL
    const timestamp = Date.now();
    const sqlFilename = `2024-day1-eng-questions-${timestamp}.sql`;
    const scraper = new AdvancedKrokScraper();
    await scraper.generateSQL(questions, sqlFilename, 2024, 'medical', '2024-day1-eng', '2024р 1 день (Eng)');
