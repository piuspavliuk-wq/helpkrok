...
async function scrapeWithSimpleLetters(url) {
  console.log('🚀 Запуск браузера з простими літерами для варіантів...');
  ...
  try {
    ...
    console.log('\n⏰ ТАЙМЕР 15 СЕКУНД ДЛЯ РУЧНОГО НАЛАШТУВАННЯ');
    ...
    console.log('6. Чекайте завершення таймера...\n');
    ...
    for (let i = 1; i <= totalQuestions; i++) {
      ...
      try {
        ...
        // Збираємо дані з простими літерами
        ...
        if (questionData && questionData.options.length > 0) {
          ...
          console.log(`✅ Питання ${i}: зібрано ${questionData.options.length} варіантів, правильна: ${questionData.correct_answer || 'не визначено'}`);
          successCount++;
        } else {
    ...
    const filename = `krok-lead-simple-letters-${timestamp}.json`;
    ...
    console.log(`📊 Зібрано ${questions.length} питань з простими літерами`);
...
if (!url) {
  ...
  console.log('📝 Використання: node scrape-simple-letters.js <URL>');
  process.exit(1);
}

scrapeWithSimpleLetters(url)
  .then((filepath) => {
