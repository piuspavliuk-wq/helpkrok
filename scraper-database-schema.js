...
class DatabaseSchemaScraper {
  ...
  async init() {
    console.log('🚀 Запуск браузера для збору даних згідно зі схемою бази даних...');
      ...
      try {
        ...
        const questionData = await this.page.evaluate((index) => {
          ...
          if (questions[index]) {
            ...
            // Ініціалізуємо поля для бази даних
            const dbQuestion = {
              option_a: '',
              option_b: '',
              option_c: '',
            ...
            for (let variantIndex = 0; variantIndex < variantElements.length; variantIndex++) {
              ...
              if (isCorrect) {
                ...
                dbQuestion.correct_answer = letter;
              }
              ...
              // Заповнюємо відповідні поля бази даних
              switch (letter) {
                case 'A':
                  dbQuestion.option_a = optionText;
                  if (optionImage) dbQuestion.option_a_image = optionImage;
                  break;
                case 'B':
                  dbQuestion.option_b = optionText;
                  if (optionImage) dbQuestion.option_b_image = optionImage;
                  break;
                case 'C':
                  dbQuestion.option_c = optionText;
                  if (optionImage) dbQuestion.option_c_image = optionImage;
                  break;
                case 'D':
                  dbQuestion.option_d = optionText;
                  if (optionImage) dbQuestion.option_d_image = optionImage;
                  break;
                case 'E':
                  dbQuestion.option_e = optionText;
                  if (optionImage) dbQuestion.option_e_image = optionImage;
                  break;
              }
              
              // Додаємо в загальні об'єкти для зворотної сумісності
            ...
            dbQuestion.question_type = hasImages || questionImage ? 'with_images' : 'text_only';
            
            // Додаємо додаткові поля для зворотної сумісності
            dbQuestion.options = options;
            dbQuestion.option_images = optionImages;
            dbQuestion.has_images = hasImages || !!questionImage;
            dbQuestion.question_number = index + 1;
            dbQuestion.extracted_at = new Date().toISOString();
    ...
    try {
      const info = await this.page.evaluate(() => {
        ...
        return {
          ...
          faculty: urlFaculty || (textFaculty?.toLowerCase().includes('медичний') ? 'medical' : 'pharmacy'),
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    // Створюємо дані для бази даних
    const dbQuestions = questions.map((q, index) => ({
      // Основні поля
      year: parseInt(pageInfo.year),
      faculty: pageInfo.faculty,
      test_identifier: `${pageInfo.faculty}-${pageInfo.year}-${pageInfo.test_type}`,
      title: pageInfo.title,
      description: `Питання ${index + 1} з тесту ${pageInfo.title}`,
      category: 'organic_chemistry', // Можна змінити залежно від тесту
      difficulty: 'medium', // Можна визначити на основі складності
      ...
      explanation_text: '', // Можна додати логіку для отримання пояснень
      reference_text: '', // Можна додати логіку для отримання посилань
      is_active: true,
    ...
    const filename = `krok-lead-database-schema-${Date.now()}.json`;
...
async function scrapeDatabaseSchema(url, timerSeconds = 30) {
  const scraper = new DatabaseSchemaScraper();
    ...
    if (questions.length > 0) {
      ...
      questions.slice(0, 3).forEach((q, i) => {
        ...
        ['A', 'B', 'C', 'D', 'E'].forEach(letter => {
          ...
          const hasImage = q[`option_${letter.toLowerCase()}_image`];
          const isCorrect = letter === q.correct_answer ? ' ✓' : '';
          const imageIcon = hasImage ? ' 🖼️' : '';
          if (optionText) {
            console.log(`   ${letter}: ${optionText.substring(0, 50)}...${isCorrect}${imageIcon}`);
          }
        });
      });
...
if (require.main === module) {
  ...
  scrapeDatabaseSchema(url, timerSeconds);
}

module.exports = { DatabaseSchemaScraper, scrapeDatabaseSchema };
