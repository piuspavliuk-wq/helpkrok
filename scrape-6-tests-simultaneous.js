...
// Конфігурація для 6 тестів з таймерами (правильний порядок)
const testsConfig = [
  { year: 2024, identifier: '2024-pharmacy-2d', title: '2024р 2д', timerSeconds: 15 },
  { year: 2024, identifier: '2024-pharmacy-eng', title: '2024 рік (Eng)', timerSeconds: 60 },
  { year: 2025, identifier: '2025-pharmacy-february', title: '2025 лютий', timerSeconds: 105 },
  { year: 2025, identifier: '2025-pharmacy-new-ct', title: 'Нові тести з цт 2025', timerSeconds: 150 },
  { year: 2025, identifier: '2025-pharmacy', title: '2025 рік', timerSeconds: 195 }
];

class PharmacyScraperSimultaneous {
...
async function runAllScrapersSimultaneously() {
  console.log('🚀 ЗАПУСКАЄМО 6 СКРАПЕРІВ ОДНОЧАСНО З ТАЙМЕРАМИ +45 СЕКУНД...\n');
  
  // Запускаємо всі скрапери одночасно
  const scraperPromises = testsConfig.map(async (testConfig) => {
    const scraper = new PharmacyScraperSimultaneous(testConfig);
    return scraper.run();
  });
  
  console.log(`✅ ВСІ ${testsConfig.length} СКРАПЕРІВ ЗАПУЩЕНО ОДНОЧАСНО!`);
  console.log('📋 Таймери:');
  testsConfig.forEach((test, index) => {
    const minutes = Math.floor(test.timerSeconds / 60);
    const seconds = test.timerSeconds % 60;
    console.log(`${index + 1}. ${test.title}: ${minutes}:${seconds.toString().padStart(2, '0')}`);
  });
  console.log('\n💡 Налаштуйте правильні відповіді в кожному браузері!');
  
  // Чекаємо завершення всіх скраперів
  ...
  results.forEach((result, index) => {
    if (result) {
      console.log(`${index + 1}. ${testsConfig[index].title}: ${result.questionsCount} питань ✅`);
      console.log(`   📄 JSON: ${result.jsonFilepath}`);
      console.log(`   📄 SQL: ${result.sqlFilepath}`);
    } else {
      console.log(`${index + 1}. ${testsConfig[index].title}: НЕ ВДАЛОСЯ ЗАВЕРШИТИ ❌`);
    }
  });
...
if (require.main === module) {
  runAllScrapersSimultaneously().catch(console.error);
}

module.exports = { PharmacyScraperSimultaneous, runAllScrapersSimultaneously };
