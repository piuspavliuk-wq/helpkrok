...
class DelayedPharmacyScraper {
  ...
  async waitWithTimer(seconds) {
    console.log(`\n⏰ ТАЙМЕР ${seconds} СЕКУНД ДЛЯ РУЧНОГО НАЛАШТУВАННЯ (${this.testConfig.title})`);
    ...
    for (let i = seconds; i > 0; i--) {
      console.log(`⏳ Залишилось: ${i} секунд... (${this.testConfig.title})`);
  ...
  async run() {
    try {
      ...
      // ТАЙМЕР 60 СЕКУНД ПЕРЕД ПОЧАТКОМ РОБОТИ
      await this.waitWithTimer(60);
...
async function runAllScrapersWithDelay() {
  console.log('🚀 ЗАПУСКАЄМО 7 СКРАПЕРІВ З ТАЙМЕРОМ 60 СЕКУНД ПЕРЕД ПОЧАТКОМ РОБОТИ...');
  console.log('');
  
  // Запускаємо всі скрапери одночасно
  const promises = tests.map((test, index) => {
    console.log(`🎯 Запускаємо скрапер ${index + 1}/7: ${test.title}`);
    const scraper = new DelayedPharmacyScraper(test);
    return scraper.run();
  });
  
  console.log('\n⏰ ВСІ 7 СКРАПЕРІВ ЗАПУЩЕНО!');
  console.log('📋 Кожен скрапер має таймер 60 секунд для налаштування');
  console.log('📋 Швидкі інструкції:');
  console.log('1. Знайдіть кнопку "три крапки" (⋮) в правому верхньому куті');
  console.log('2. Натисніть на неї');
  console.log('3. Виберіть "Показувати правильні відповіді"');
  ...
  console.log('6. Чекайте завершення таймерів...\n');
...
if (require.main === module) {
  runAllScrapersWithDelay().catch(console.error);
}

module.exports = { DelayedPharmacyScraper, runAllScrapersWithDelay };
