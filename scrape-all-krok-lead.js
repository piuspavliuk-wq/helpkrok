...
class CompleteKrokLeadScraper {
  ...
  async init() {
    console.log('🚀 Запуск браузера для повного збору питань...');
  ...
  async saveQuestions(questions, pageInfo) {
    ...
    const filename = `krok-lead-complete-${Date.now()}.json`;
...
// Основна функція
async function scrapeAllKrokLead(url) {
  const scraper = new CompleteKrokLeadScraper();
...
if (require.main === module) {
  ...
  scrapeAllKrokLead(url);
}

module.exports = { CompleteKrokLeadScraper, scrapeAllKrokLead };
