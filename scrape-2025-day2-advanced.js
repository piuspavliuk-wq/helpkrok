...
async function scrapeAdvanced(url) {
  ...
  try {
    ...
    const jsonFilename = `2025-day2-advanced-questions-${timestamp}.json`;
...
async function main() {
  console.log('🎯 РОЗШИРЕНИЙ СКРАПЕР для тесту "2025р 2 д"');
  ...
  if (questions.length > 0) {
    ...
    // Генеруємо SQL
    const timestamp = Date.now();
    const sqlFilename = `2025-day2-advanced-questions-${timestamp}.sql`;
    const scraper = new AdvancedKrokScraper();
