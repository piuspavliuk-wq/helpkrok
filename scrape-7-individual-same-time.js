...
// Конфігурація тестів - всі з однаковим таймером
const tests = [
  {
    year: 2019,
...
class IndividualPharmacyScraper {
...
async function runAllScrapersIndividual() {
  console.log('🚀 ЗАПУСКАЄМО 7 ОКРЕМИХ СКРАПЕРІВ ОДНАКОВИХ АЛЕ ОДНОЧАСНО...');
  console.log('');
  
  // Запускаємо всі скрапери одночасно
  const promises = tests.map((test, index) => {
    console.log(`🎯 Запускаємо скрапер ${index + 1}/7: ${test.title} (таймер: ${test.timerSeconds}с)`);
    const scraper = new IndividualPharmacyScraper(test);
    return scraper.run();
  });
  
  console.log('\n⏰ ВСІ 7 СКРАПЕРІВ ЗАПУЩЕНО ОДНОЧАСНО! Чекаємо завершення...\n');
...
if (require.main === module) {
  runAllScrapersIndividual().catch(console.error);
}

module.exports = { IndividualPharmacyScraper, runAllScrapersIndividual };
