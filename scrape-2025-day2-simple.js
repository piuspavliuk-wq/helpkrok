...
async function scrapeSimple(url) {
  ...
  try {
    ...
    const jsonFilename = `2025-day2-simple-questions-${timestamp}.json`;
...
async function main() {
  console.log('🎯 Скрапер для тесту "2025р 2 д" (спрощена версія)');
  ...
  const questions = await scrapeSimple(url);
  ...
  if (questions.length > 0) {
    ...
    // Генеруємо SQL
    const timestamp = Date.now();
    const sqlFilename = `2025-day2-simple-questions-${timestamp}.sql`;
